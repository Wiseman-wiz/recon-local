...
@csrf_exempt
@login_required(login_url="/login/")
def bank_recon_records_deletion(request):
    context = {}
...
@csrf_exempt
@login_required(login_url="/login/")
def bank_recon_records_summary(request):
    context = {}
...
def bank_recon_records(request, collection):  # complexity 8
    ...
    if request.method == "POST":
        if "btn_approval" in request.POST:
            ...
            rc.delete_records(request, db, request.POST.get("collection"))
        else:
    ...
    for data in records_data:
        ...
        for key, val in data.items():
            if 'date' in key.lower():
                serialized_data[key] = val.strftime('%m/%d/%Y') if isinstance(val, datetime.datetime) else val
            # commented line for now (refactoring)
            # elif 'amount' in key.lower() or 'amt' in key.lower():
                # serialized_data[key] = '{:,.2f}'.format(val)
            else:
                serialized_data[key] = val
        
        if data["approved"] == "approved":
            context["approved"].append(serialized_data)
        elif data["approved"] is False:
            context["rejected"].append(serialized_data)
        elif data["approved"] == "pending":
            context["pending"].append(serialized_data)
            
    paginator = Paginator(context["pending"], per_page=50)  # Adjust per_page as needed
    ...
    print("VIEW TIME >>>>>>>>> ", (end - start) * 10**3, "ms")
    return HttpResponse(html_template.render(context, request))
